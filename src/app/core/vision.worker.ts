...
interface Request { id: number; image: ImageData; guide: Region; titleOnly?: boolean }
...
function cropTitle(image:ImageData,region:Region):{image:ImageData;geometry:CardGeometry}{const x=Math.max(0,Math.round(region.x*image.width)),y=Math.max(0,Math.round(region.y*image.height)),width=Math.max(1,Math.min(image.width-x,Math.round(region.width*image.width))),height=Math.max(1,Math.min(image.height-y,Math.round(region.height*image.height))),out=new ImageData(width,height);for(let row=0;row<height;row++){const start=((y+row)*image.width+x)*4;out.data.set(image.data.subarray(start,start+width*4),row*width*4);}return{image:out,geometry:{corners:[{x,y},{x:x+width,y},{x:x+width,y:y+height},{x,y:y+height}],sourceWidth:image.width,sourceHeight:image.height,canonicalWidth:900,canonicalHeight:1257,score:1}}}
...
addEventListener('message',({data}:MessageEvent<Request>)=>{try{const title=cropTitle(data.image,data.guide),processed=variants(title.image);postMessage({id:data.id,geometry:title.geometry,variants:processed} satisfies Response,Object.values(processed).map(v=>v.data.buffer));}catch(error){postMessage({id:data.id,error:error instanceof Error?error.message:'Preparazione del titolo non riuscita.'} satisfies Response);}});
